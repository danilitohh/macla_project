...
const normalizeProductSnapshot = (snapshot, { id, name, unitPrice }) => {
  const override = id && PRODUCT_OVERRIDES[id] ? PRODUCT_OVERRIDES[id] : null;
  const baseSnapshot = override
    ? {
        ...(snapshot && typeof snapshot === "object" ? snapshot : {}),
        ...override,
        name: override.name || snapshot?.name,
      }
    : snapshot;

  const normalizedId = baseSnapshot?.id || id || null;
  const normalizedName =
    baseSnapshot?.name ||
    name ||
    (normalizedId ? `Producto ${normalizedId}` : "Producto");
  const normalizedPrice = safeInteger(baseSnapshot?.price, unitPrice);
  const normalizedImages = Array.isArray(baseSnapshot?.images)
    ? baseSnapshot.images.filter(Boolean)
    : baseSnapshot?.image
      ? [baseSnapshot.image]
  ...
  return {
    ...
    currency: baseSnapshot?.currency || "COP",
...
const fetchDiscountByCode = async (code) => {
  ...
  const [rows] = await query(SQL.selectDiscountByCodeAny, [normalized]);
  const row = rows[0];
...
const calculateDiscount = ({ config, subtotalCents = 0, shippingCents = 0 }) => {
  if (!config) {
    return {
      valid: false,
      code: "",
  ...
  if (config.min_subtotal_cents && subtotalCents < config.min_subtotal_cents) {
    return {
      valid: false,
      code: config.code,
      discountCents: 0,
      shippingDiscountCents: 0,
      reason: `Subtotal mínimo ${config.min_subtotal_cents} requerido`,
    };
  }
  ...
  if (config.type === "percent") {
    discountCents = Math.floor(
      (subtotalCents * Number(config.percent_value || 0)) / 100,
    );
    if (config.max_discount_cents) {
      discountCents = Math.min(discountCents, config.max_discount_cents);
    }
  } else if (config.type === "flat") {
    discountCents = Math.min(config.value_cents, subtotalCents);
  } else if (config.type === "shipping") {
    const cap = config.value_cents || shippingCents || 0;
    shippingDiscountCents = Math.min(shippingCents || cap, cap);
  }
  ...
  return {
    ...
    label: config.label || config.code,
    type: config.type,
  };
};
...
const WOMPI_ENV =
  process.env.WOMPI_ENV === "production" ? "production" : "sandbox";
...
const wompiKeyEnvironment = () => {
  if (
    ...
    WOMPI_PRIVATE_KEY.startsWith("prv_prod_")
  ) {
    return "production";
  }
  if (
    WOMPI_PUBLIC_KEY.startsWith("pub_test_") ||
...
const toProductDto = (row, extras) => {
  ...
  const content =
    row.content_json && typeof row.content_json === "string"
      ? parseJsonSafe(row.content_json) || null
      : null;
...
const fetchProductById = async (
  ...
  const [rows] = await query(
    includeInactive ? SQL.selectProductByIdAny : SQL.selectProductByIdActive,
    [productId],
  );
  const row = rows[0];
  if (!row) {
...
const saveProductWithExtras = async (payload) => {
  ...
  for (let i = 0; i < images.length; i += 1) {
    ...
    if (typeof url === "string" && url.startsWith("data:")) {
      try {
        const stored = await dataUrlToFilePath(url, {
          prefix: `product-${id || "item"}-${i}`,
        });
        if (stored) {
        ...
        console.error(
          "[saveProductWithExtras] error saving data URL to file",
          err,
        );
  ...
  try {
    ...
    await connection.execute("DELETE FROM product_specs WHERE product_id = ?", [
      id,
    ]);
...
const fetchAnnouncements = async ({ includeInactive = false } = {}) => {
  const [rows] = await query(
    includeInactive ? SQL.selectAllAnnouncements : SQL.selectAnnouncements,
  );
...
const normalizeProductPayload = (body, { enforceId } = {}) => {
  ...
  const currency =
    sanitizeText(body?.currency || DEFAULT_CURRENCY).toUpperCase() ||
    DEFAULT_CURRENCY;
  const idSource = enforceId
    ? body?.id
    : body?.id || body?.slug || body?.sku || name;
  const id = enforceId
    ? normalizeIdentifier(idSource || body?.id, "product")
    : normalizeIdentifier(idSource, "product");
  const isActive = body?.isActive !== undefined ? Boolean(body.isActive) : true;
  const content =
    body && typeof body.content === "object" ? body.content : null;
...
const upsertOrderPayment = async ({
  ...
  const nowSql = formatDateForSql(new Date());
  const processedAt =
    status === "completed" || status === "authorized" ? nowSql : null;
...
const fetchWompiTransaction = async (transactionId) => {
  ...
  const response = await fetch(
    `${WOMPI_BASE_URL}/v1/transactions/${transactionId}`,
    {
      headers: {
  ...
  if (!response.ok) {
    ...
    throw new Error(
      `No se pudo consultar la transacción Wompi: ${response.status} ${text}`,
    );
  }
...
const markOrderPaidFromWompi = async ({
  orderId,
  orderCode,
  userId,
  transaction,
  ...
  if (status === "APPROVED") {
    ...
    await query(SQL.insertOrderStatusHistory, [
      orderId,
      "paid",
      "wompi",
...
const createVerificationToken = async ({
  ...
  const expiresMinutes =
    TOKEN_CONFIG[type] && Number.isFinite(TOKEN_CONFIG[type])
      ? TOKEN_CONFIG[type]
      : 15;
...
const SQL = {
  ...
  updateUserProfile:
    "UPDATE users SET name = ?, phone = ?, city = ?, address_line = ?, updated_at = ? WHERE id = ?",
  findActiveCart: `
  ...
  `,
  touchCart:
    "UPDATE shopping_carts SET last_activity_at = ?, updated_at = ? WHERE id = ?",
  countUsers: "SELECT COUNT(*) AS total_users FROM users",
  countOrders: "SELECT COUNT(*) AS total_orders FROM orders",
  countActiveCarts:
    "SELECT COUNT(*) AS active_carts FROM shopping_carts WHERE status = 'active'",
  countAbandonedCarts:
    "SELECT COUNT(*) AS abandoned_carts FROM shopping_carts WHERE status = 'abandoned'",
  recentCarts: `
    SELECT sc.id, sc.user_id, sc.status, sc.total_items, sc.total_cents, sc.currency,
  ...
  clearDefaultShipping:
    "UPDATE user_addresses SET is_default_shipping = 0 WHERE user_id = ?",
  clearDefaultBilling:
    "UPDATE user_addresses SET is_default_billing = 0 WHERE user_id = ?",
  ...
  `,
  consumeTokenById:
    "UPDATE user_verification_tokens SET consumed_at = ?, updated_at = ? WHERE id = ?",
  deleteExpiredTokens:
    "DELETE FROM user_verification_tokens WHERE expires_at < ?",
  selectDiscounts: `
    SELECT code, type, value_cents, percent_value, min_subtotal_cents, max_discount_cents, label, is_active, created_at, updated_at
    FROM discount_codes
    ORDER BY updated_at DESC
  `,
  selectDiscountByCodeAny: `
    SELECT code, type, value_cents, percent_value, min_subtotal_cents, max_discount_cents, label, is_active
    FROM discount_codes
    WHERE code = ?
    LIMIT 1
  `,
  upsertDiscount: `
    INSERT INTO discount_codes (
      code, type, value_cents, percent_value, min_subtotal_cents, max_discount_cents, label, is_active
    ) VALUES (?, ?, ?, ?, ?, ?, ?, ?)
    ON DUPLICATE KEY UPDATE
      type = VALUES(type),
      value_cents = VALUES(value_cents),
      percent_value = VALUES(percent_value),
      min_subtotal_cents = VALUES(min_subtotal_cents),
      max_discount_cents = VALUES(max_discount_cents),
      label = VALUES(label),
      is_active = VALUES(is_active),
      updated_at = CURRENT_TIMESTAMP(3)
  `,
  deleteDiscount: `DELETE FROM discount_codes WHERE code = ?`,
  selectActiveProducts: `
    SELECT id, category_id, name, short_description, description, price_cents, currency, stock, is_active, content_json
    FROM products
    WHERE is_active = 1
    ORDER BY updated_at DESC
  `,
  ...
  rows
    .map((row) => {
      ...
      const unitPrice = safeInteger(
        product?.price,
        safeInteger(row.unit_price_cents, 0),
      );
      ...
      return {
        ...
        lineTotal: safeInteger(
          ...
          unitPrice * safeInteger(row.quantity, 0),
        ),
      };
    })
    .filter(Boolean);
...
app.get("/api/payments/wompi/config", requireAuth, async (req, res) => {
  ...
  if (!configured) {
    ...
    console.error(
      "[wompi] WOMPI_PUBLIC_KEY no está configurada en el servidor.",
    );
  }

  const keyEnvMismatch =
    WOMPI_KEY_ENV !== "unknown" && WOMPI_KEY_ENV !== WOMPI_ENV;
  if (configured && keyEnvMismatch) {
    ...
    console.error("[wompi] clave y entorno no coinciden:", {
      WOMPI_ENV,
      WOMPI_KEY_ENV,
    });
  }
  ...
  return res.json({
    ...
    message: configured
      ? null
      : message || "Wompi no está configurado en el servidor.",
    integrityRequired: Boolean(WOMPI_INTEGRITY_SECRET),
    signatureHelp: WOMPI_INTEGRITY_SECRET
      ? null
      : "Configura WOMPI_INTEGRITY_SECRET para generar la firma de integridad.",
  });
});
...
app.post("/api/payments/wompi/verify", requireAuth, async (req, res, next) => {
  try {
    ...
    if (!transactionId || !orderCode) {
      return res
        .status(400)
        .json({ message: "Faltan datos para validar el pago." });
    }
    ...
    if (!order) {
      return res
        .status(404)
        .json({ message: "Pedido no encontrado para este usuario." });
    }
    ...
    if (!wompiTx) {
      return res
        .status(400)
        .json({ message: "No pudimos validar la transacción en Wompi." });
    }
    if (wompiTx.reference !== orderCode) {
      return res
        .status(400)
        .json({ message: "La referencia de pago no coincide con el pedido." });
    }
...
app.post("/api/payments/wompi/signature", requireAuth, async (req, res) => {
  ...
  if (!WOMPI_INTEGRITY_SECRET) {
    return res.status(503).json({
      message: "El servidor no tiene WOMPI_INTEGRITY_SECRET configurado.",
    });
  }
  ...
  if (!order) {
    return res
      .status(404)
      .json({ message: "Pedido no encontrado para generar la firma." });
  }
...
app.post("/api/auth/register", async (req, res, next) => {
  try {
    ...
    if (!email || !email.includes("@")) {
      return res
        .status(400)
        .json({ message: "Debes ingresar un correo válido." });
    }
    ...
    if (existingRows.length > 0) {
      return res
        .status(409)
        .json({ message: "Ya existe una cuenta registrada con este correo." });
    }
...
app.post("/api/auth/activate", async (req, res, next) => {
  try {
    ...
    if (!email) {
      return res
        .status(400)
        .json({ message: "Debes indicar el correo de la cuenta." });
    }
    ...
    if (!user) {
      return res
        .status(404)
        .json({ message: "No encontramos una cuenta con este correo." });
    }
    ...
    return res.json({
      alreadyActive: true,
      message:
        "La activación ya no es necesaria. Inicia sesión con tu correo y contraseña.",
    });
  } catch (error) {
    return next(error);
  }
...
app.post("/api/auth/resend-activation", async (req, res, next) => {
  try {
    ...
    if (!email) {
      return res
        .status(400)
        .json({ message: "Debes ingresar el correo registrado." });
    }
    const [userRows] = await query(SQL.userByEmail, [email]);
    const user = userRows[0];
    if (!user) {
      return res.json({
        message: "Si la cuenta existe, ya puede iniciar sesión sin códigos.",
      });
    }
  ...
  try {
    ...
    if (!email && !phone) {
      return res.status(400).json({
        message: "Ingresa el correo o teléfono asociado a tu cuenta.",
      });
    }
    ...
    if (!user) {
      return res.json({
        message:
          "Si encontramos tu cuenta, enviaremos instrucciones de recuperación.",
      });
    }
    ...
    return res.json({
      message:
        "Si encontramos tu cuenta, enviamos los pasos para restablecer tu contraseña.",
      channel,
...
app.post("/api/auth/reset-password", async (req, res, next) => {
  try {
    ...
    if (!email || password.length < 6) {
      return res.status(400).json({
        message:
          "Debes ingresar correo y una contraseña válida (mínimo 6 caracteres).",
      });
    }
    if (!token && !code) {
      return res.status(400).json({
        message: "Ingresa el código o token recibido para continuar.",
      });
    }
    ...
    if (!verification) {
      return res
        .status(400)
        .json({ message: "El código ingresado no es válido o ya expiró." });
    }
...
app.put("/api/profile", requireAuth, async (req, res, next) => {
  try {
    ...
    const nowSql = formatDateForSql(new Date());
    await query(SQL.updateUserProfile, [
      name,
      phone || null,
      city || null,
      address || null,
...
app.post("/api/addresses", requireAuth, async (req, res, next) => {
  try {
    ...
    const contactPhone = sanitizePhone(
      payload.contactPhone || req.authUser?.phone,
    );
    ...
    if (!label || !contactName || !contactPhone || !city || !address) {
      return res.status(400).json({
        message: "Completa nombre, teléfono, ciudad y dirección para guardar.",
      });
    }
    ...
    const [rows] = await query(SQL.userAddressById, [
      result.insertId,
      req.userId,
    ]);
    ...
    return res
      .status(201)
      .json({ address: created ? toAddressDto(created) : null });
  } catch (error) {
    return next(error);
  }
});
...
app.put("/api/addresses/:id", requireAuth, async (req, res, next) => {
  try {
    ...
    const [existingRows] = await query(SQL.userAddressById, [
      addressId,
      req.userId,
    ]);
    ...
    const contactName = sanitizeName(
      payload.contactName || existing.contact_name,
    );
    const contactPhone = sanitizePhone(
      payload.contactPhone || existing.contact_phone,
    );
    ...
    const isDefaultShipping = payload.isDefaultShipping
      ? 1
      : existing.is_default_shipping;
    const isDefaultBilling = payload.isDefaultBilling
      ? 1
...
app.put("/api/cart", requireAuth, async (req, res, next) => {
  ...
  try {
    ...
    const [existingRows] = await connection.execute(SQL.findActiveCart, [
      req.userId,
    ]);
    let cartId;
    ...
    if (existingRows.length > 0) {
      ...
      cartId =
        typeof crypto.randomUUID === "function"
          ? crypto.randomUUID()
          : crypto.randomBytes(16).toString("hex");
      await connection.execute(SQL.createCart, [
        cartId,
        req.userId,
        cartCurrency,
        nowString,
    ...
    await connection.execute(SQL.deactivateOtherCarts, [
      nowString,
      nowString,
      req.userId,
      cartId,
...
app.post("/api/discounts/validate", requireAuth, async (req, res) => {
  ...
  const code = normalizeDiscountCode(
    req.body?.code || req.body?.discountCode || "",
  );

  if (!code) {
    return res
      .status(400)
      .json({ message: "Ingresa un código para validarlo." });
  }

  const config = await fetchDiscountByCode(code);
  const result = calculateDiscount({ config, subtotalCents, shippingCents });
  if (!result.valid) {
    return res
      .status(400)
      .json({ message: "El código no es válido para este pedido." });
  }

  const totalDiscountCents =
    result.discountCents + result.shippingDiscountCents;
  const adjustedShipping = Math.max(
    0,
    shippingCents - result.shippingDiscountCents,
  );
  const estimatedTotal = Math.max(
    0,
    subtotalCents - result.discountCents + adjustedShipping,
  );
...
app.post("/api/orders", requireAuth, async (req, res, next) => {
  ...
  const shippingOptionIdRaw =
    payload.shippingOptionId || payload.shippingOption?.id || "";
  const paymentMethodIdRaw =
    payload.paymentMethodId || payload.paymentMethod?.id || "";
  const shippingOptionId = sanitizeText(shippingOptionIdRaw) || null;
  const paymentMethodId = sanitizeText(paymentMethodIdRaw) || null;
  const discountCodeRaw = sanitizeText(
    payload.discountCode || payload.coupon || "",
  );
  ...
  if (addressId > 0) {
    const [addressRows] = await query(SQL.userAddressById, [
      addressId,
      req.userId,
    ]);
    ...
    if (!addressRecord) {
      return res
        .status(404)
        .json({ message: "La dirección seleccionada no existe." });
    }
  }

  const customerName = sanitizeName(
    customer.name || addressRecord?.contact_name,
  );
  const customerPhone = sanitizeText(
    customer.phone || addressRecord?.contact_phone,
  );
  const customerCity = sanitizeText(customer.city || addressRecord?.city);
  const customerAddress = sanitizeText(
    customer.address || addressRecord?.address_line,
  );
  const customerNotesRaw = sanitizeText(
    customer.notes || addressRecord?.notes || "",
  );
  ...
  if (!customerPhone) {
    return res
      .status(400)
      .json({ message: "Debes ingresar un teléfono de contacto." });
  }
  if (!customerCity) {
    return res
      .status(400)
      .json({ message: "Debes indicar la ciudad para el envío." });
  }
  if (!customerAddress) {
    return res
      .status(400)
      .json({ message: "Debes indicar la dirección de entrega." });
  }
  ...
  try {
    ...
    const [cartRows] = await connection.execute(SQL.findActiveCart, [
      req.userId,
    ]);
    ...
    const addNormalizedItem = ({
      ...
      const unitPrice = productDescriptor.price;
      const qty = safeInteger(quantity, 0);
      ...
      const snapshotJson = JSON.stringify(productDescriptor);

      orderItemsForInsert.push({
        productId,
        productName: productDescriptor.name,
        productSku,
        unitPrice,
        quantity: qty,
        lineTotal,
    ...
    // 1) Usar siempre los items que llegan del cliente si están presentes
    if (payloadItems.length > 0) {
      payloadItems.forEach((item) => {
        ...
        const unitPriceFallback = safeInteger(product?.price, 0);
    ...
    // 2) Si no hay items en el payload, tomamos el carrito activo en BD
    if (orderItemsForInsert.length === 0 && cart) {
      const [itemRows] = await connection.execute(SQL.selectCartItems, [
        cart.id,
      ]);
      ...
      for (const row of itemRows) {
        ...
        const rawUnitPrice = safeInteger(row.unit_price_cents, 0);
        const snapshot = parseJsonSafe(row.product_snapshot_json);
        const productId =
          row.product_id || (snapshot?.id ? String(snapshot.id) : null);
        const productName =
          snapshot?.name ||
          snapshot?.title ||
          (productId ? `Producto ${productId}` : "Producto");
        ...
        addNormalizedItem({
          ...
          snapshot,
          quantity,
          unitPriceFallback: rawUnitPrice,
        });
      }
    }
    ...
    if (orderItemsForInsert.length === 0 || subtotalCents <= 0) {
      await connection.rollback();
      return res.status(400).json({
        message: "Tu carrito está vacío o no pudimos procesar los productos.",
      });
    }
    ...
    if (shippingOptionId) {
      const [shippingRows] = await connection.execute(SQL.shippingOptionById, [
        shippingOptionId,
      ]);
      ...
      if (!shippingOption) {
        ...
        return res
          .status(400)
          .json({ message: "La opción de envío seleccionada no es válida." });
      }
    ...
    if (paymentMethodId) {
      const [paymentRows] = await connection.execute(SQL.paymentMethodById, [
        paymentMethodId,
      ]);
      ...
      if (!paymentMethod) {
        ...
        return res
          .status(400)
          .json({ message: "El método de pago seleccionado no es válido." });
      }
    ...
    if (discountCode) {
      const discountConfig = await fetchDiscountByCode(discountCode);
      ...
      if (!calc.valid) {
        await connection.rollback();
        return res.status(400).json({
          message: "El código de descuento no es válido o no aplica al pedido.",
        });
      }
    ...
    const shippingCostFinalCents = Math.max(
      0,
      shippingCostCents - discountDetails.shippingDiscountCents,
    );
    const totalDiscountCents =
      discountDetails.discountCents +
      (shippingCostCents - shippingCostFinalCents);
    const totalCents = Math.max(
      0,
      subtotalCents - discountDetails.discountCents + shippingCostFinalCents,
    );
    ...
    if (cartId) {
      await connection.execute(SQL.markCartSubmitted, [
        "submitted",
        nowSql,
        nowSql,
        nowSql,
      ...
      orderRows.map(async (order) => {
        ...
        return {
          ...
          submittedAt: order.submitted_at
            ? new Date(order.submitted_at).toISOString()
            : null,
  ...
  async (req, res, next) => {
    try {
      ...
      if (!payload.title || !payload.description) {
        return res
          .status(400)
          .json({ message: "El anuncio necesita título y descripción." });
      }
...
// Admin discounts
const normalizeDiscountPayload = (body) => {
  const code = normalizeDiscountCode(body?.code || "");
  const type = ["percent", "flat", "shipping"].includes(String(body?.type))
    ? String(body.type)
    : "percent";
  const percentValue = Math.max(
    0,
    Math.min(100, Number(body?.percent_value ?? body?.percent ?? 0)),
  );
  const valueCents = safeInteger(body?.value_cents ?? body?.value ?? 0, 0);
  const minSubtotalCents = safeInteger(
    body?.min_subtotal_cents ?? body?.minSubtotal ?? 0,
    0,
  );
  const maxDiscountCents = safeInteger(
    body?.max_discount_cents ?? body?.maxDiscount ?? 0,
    0,
  );
...
app.get(
  ...
  async (_req, res, next) => {
    try {
      const [rows] = await query(SQL.selectDiscounts);
...
app.post(
  ...
  async (req, res, next) => {
    try {
      ...
      await query(SQL.upsertDiscount, [
        payload.code,
        payload.type,
        payload.value_cents,
        payload.percent_value,
        payload.min_subtotal_cents,
        payload.max_discount_cents,
        payload.label,
        payload.is_active,
      ]);
      const [rows] = await query(SQL.selectDiscountByCodeAny, [payload.code]);
      return res.status(201).json({ discount: rows[0] || payload });
    } catch (error) {
      return next(error);
    }
  },
...
app.delete(
  ...
  async (req, res, next) => {
    try {
      ...
      await query(SQL.deleteDiscount, [code]);
      return res.status(204).send();
    } catch (error) {
      return next(error);
    }
...
try {
  canServeClient =
    fs.existsSync(CLIENT_DIST_PATH) && fs.existsSync(CLIENT_INDEX_PATH);
} catch (_error) {
...
app.use((err, _req, res, _next) => {
  ...
  if (err?.code === "ER_DATA_TOO_LONG") {
    status = 400;
    message =
      "El archivo o texto es demasiado grande. Reduce el peso de la imagen o el tamaño del contenido.";
  } else if (isPayloadTooLarge) {
    message =
      "La solicitud es demasiado grande. Usa imágenes más livianas o reduce el contenido.";
  } else if (
    status >= 400 &&
    status < 500 &&
